...
const BetRow = ({ market, bets, selectedBetId, baseLine }) => {
  ...
  useEffect(() => {
    ...
    if (marketInfo) {
      ...
      if (homeBet)
        arr.push(
          TabTrigger(homeBet.Id, home === "1" ? "Home" : home, homeBet.Price),
        );
      if (!drawBet && baseLine) {
        arr.push(
          <div
      ...
      if (drawBet)
        arr.push(
          TabTrigger(drawBet.Id, draw === "X" ? "Draw" : draw, drawBet.Price),
        );

      if (awayBet)
        arr.push(
          TabTrigger(awayBet.Id, away === "2" ? "Away" : away, awayBet.Price),
        );
    }
